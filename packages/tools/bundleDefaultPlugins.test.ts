...
import { createTempDir, supportDir } from '@xilinota/lib/testing/test-utils';
...
const manifests = {
	...
	'plugin.calebjohn.rich-markdown': {
		...
		'author': 'Caleb John',
		'homepage_url': 'https://github.com/CalebJohn/xilinota-rich-markdown#readme',
		'repository_url': 'https://github.com/CalebJohn/xilinota-rich-markdown',
		'keywords': [
			'editor',
			'visual',
		],
...
describe('bundleDefaultPlugins', () => {
	...
	it('should get local plugin versions', async () => {
		...
		const testDefaultPluginsInfo = {
			'xilinota.plugin.ambrt.backlinksToNote': { version: '1.0.4' },
			'org.xilinotaapp.plugins.ToggleSidebars': { version: '1.0.2' },
		};
		const localPluginsVersions = await localPluginsVersion(manifestsPath, testDefaultPluginsInfo);

		expect(localPluginsVersions['xilinota.plugin.ambrt.backlinksToNote']).toBe('1.0.4');
		expect(localPluginsVersions['org.xilinotaapp.plugins.ToggleSidebars']).toBe('1.0.2');
	});
