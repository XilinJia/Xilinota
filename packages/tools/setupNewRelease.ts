...
function isXilinotaPackage(name: string): boolean {
	if (!name.startsWith('@xilinota/')) return false;
	if (['@xilinota/turndown', '@xilinota/turndown-plugin-gfm'].includes(name)) return false;
	return !name.startsWith('@xilinota/fork-');
}
...
async function main() {
	...
	await updatePackageVersion(`${rootDir}/packages/generator-xilinota/package.json`, majorMinorVersion, options);
	...
	if (options.updateVersion) {
		...
		await updateCodeProjVersion(`${rootDir}/packages/app-mobile/ios/Xilinota.xcodeproj/project.pbxproj`, iosVersionHack(majorMinorVersion));
		await updateClipperManifestVersion(`${rootDir}/packages/app-clipper/manifest.json`, majorMinorVersion);
		await updatePluginGeneratorTemplateVersion(`${rootDir}/packages/generator-xilinota/generators/app/templates/src/manifest.json`, majorMinorVersion);
	}
