...
import { hasCredentialFile, readCredentialFile } from '@xilinota/lib/utils/credentialFiles';
...
async function insertChangelog(tag: string, changelogPath: string, changelog: string, isPrerelease: boolean, repoTagUrl = '') {
	repoTagUrl = repoTagUrl || 'https://github.com/XilinJia/Xilinota/releases/tag';
...
export async function githubUsername(email: string, name: string) {
	...
	const urlsToTry = [
		...
		// it for now.
		// https://github.com/XilinJia/Xilinota/pull/5390
...
// Note that the GitHub API releases/latest is broken on the xilinota-android repo
...
export async function gitHubLatestRelease(repoName: string): Promise<GitHubRelease> {
	const response: any = await fetch(`https://api.github.com/repos/xilinjia/${repoName}/releases/latest`, {
		headers: {
			'Content-Type': 'application/json',
			'User-Agent': 'Xilinota Readme Updater',
		},
	});
...
export async function gitHubLatestRelease_KeepInCaseMicrosoftBreaksTheApiAgain(repoName: string): Promise<GitHubRelease> {
	...
	while (true) {
		const response: any = await fetch(`https://api.github.com/repos/xilinjia/${repoName}/releases?page=${pageNum}`, {
			headers: {
				'Content-Type': 'application/json',
				'User-Agent': 'Xilinota Readme Updater',
			},
...
export const gitHubLatestReleases = async (page: number, perPage: number) => {
	const response = await fetch(`https://api.github.com/repos/xilinjia/xilinota/releases?page=${page}&per_page=${perPage}`, {
		headers: {
			'Content-Type': 'application/json',
			'User-Agent': 'Xilinota Forum Updater',
		},
	});
...
export async function githubRelease(project: string, tagName: string, options: any = null): Promise<GitHubRelease> {
	...
	const response = await fetch(`https://api.github.com/repos/xilinjia/${project}/releases`, {
		method: 'POST',
		body: JSON.stringify({
			tag_name: tagName,
			name: tagName,
...
export const gitHubLinkify = (markdown: string) => {
	markdown = markdown.replace(/#(\d+)/g, '[#$1](https://github.com/XilinJia/Xilinota/issues/$1)');
	markdown = markdown.replace(/\(([a-f0-9]+)\)/g, '([$1](https://github.com/XilinJia/Xilinota/commit/$1))');
	return markdown;
};
