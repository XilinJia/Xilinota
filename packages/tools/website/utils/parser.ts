...
export function replaceGitHubByWebsiteLinks(md: string) {
	return md
		.replace(/https:\/\/github.com\/xilinjia\/xilinota\/blob\/dev\/readme\/(.*?)\/index\.md(#[^\s)]+|)/g, '/$1/$2')
		.replace(/https:\/\/github.com\/xilinjia\/xilinota\/blob\/dev\/readme\/(.*?)\.md(#[^\s)]+|)/g, '/$1/$2')
		.replace(/https:\/\/github.com\/xilinjia\/xilinota\/blob\/dev\/README\.md(#[^\s)]+|)/g, '/help/$1')
