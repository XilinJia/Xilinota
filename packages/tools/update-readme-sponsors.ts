...
async function createOrgSponsorTable(sponsors: OrgSponsor[]): Promise<string> {
	...
	for (const sponsor of sponsors) {
		output.push(`<a href="${escapeHtml(sponsor.url)}"><img title="${escapeHtml(sponsor.title)}" width="256" src="https://xilinotaapp.org/images/sponsors/${escapeHtml(sponsor.imageName)}"/></a>`);
	}
