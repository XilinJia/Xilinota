...
async function createSourceZip() {
	...
	const filename = 'xilinota-webclipper-source.zip';
...
async function compareDir(dir1, dir2) {
	...
	const files1 = glob.sync(`${dir1}/**/*`, globOptions).filter(filterFiles).map(f => f.substring(dir1.length + 1));
	const files2 = glob.sync(`${dir2}/**/*`, globOptions).filter(filterFiles).map(f => f.substring(dir2.length + 1));
...
async function main() {
	...
	for (const distName in dists) {
		...
		manifest.name = 'Xilinota Web Clipper';
