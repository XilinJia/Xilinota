...
export default function useXilinotaMode(CodeMirror: any): void {
	...
	CodeMirror.defineMode('xilinota-markdown', (config: any) => {
		...
		return {
			startState: function(): XilinotaModeState {
				return {
			...
			indent: function(state: XilinotaModeState, textAfter: string, line: any) {
			...
			blankLine: function(state: XilinotaModeState) {
				const mode = state.openCharacter ? stex : markdownMode;
				if (mode.blankLine) {
			...
			innerMode: function(state: XilinotaModeState) {
