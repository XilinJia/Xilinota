...
function CodeMirror(props: NoteBodyEditorProps, ref: ForwardedRef<NoteBodyEditorRef>) {
			...
			execCommand: async (cmd: EditorCommand) => {
				...
				if (!commandProcessed) {
					...
					if (commands[cmd.name]) {
						...
						commandOutput = editorRef.current.execCommandFromXilinota(cmd);
					} else {
	...
	useEffect(() => {
		...
		// https://discourse.xilinotaapp.org/t/dragging-in-dark-theme/12433/4?u=laurent
	...
	useEffect(() => {
		...
		if (webviewRef.current && (props.searchMarkers !== previousSearchMarkers || textChanged)) {
			...
			if (editorRef.current) {
				// Fixes https://github.com/XilinJia/Xilinota/issues/7565
	...
	useEffect(() => {
		...
		// that it's container size is stable, that is not true with Xilinota, hence
		...
		return (
			<div style={cellEditorStyle}>
				<Editor
					...
					ref={editorRef}
					mode={props.contentMarkupLanguage === MarkupToHtml.MARKUP_LANGUAGE_HTML ? 'xml' : 'xilinota-markdown'}
