...
import { _, closestSupportedLocale } from '@xilinota/lib/locale';
...
import { plainTextToHtml } from '@xilinota/lib/htmlUtils';
...
function findEditableContainer(node: any): any {
	while (node) {
		if (node.classList && node.classList.contains('xilinota-editable')) return node;
...
const TinyMCE = (props: NoteBodyEditorProps, ref: any) => {
	...
	const dispatchDidUpdate = (editor: any) => {
		...
		dispatchDidUpdateIID_ = shim.setTimeout(() => {
			...
			if (editor && editor.getDoc()) editor.getDoc().dispatchEvent(new Event('xilinota-noteDidUpdate'));
		}, 10);
	};
	...
	useImperativeHandle(ref, () => {
		return {
			...
			supportsCommand: (name: string) => {
				...
				return !!xilinotaCommandToTinyMceCommands[name];
			},
			execCommand: async (cmd: EditorCommand) => {
				...
				if (!xilinotaCommandToTinyMceCommands[cmd.name]) {
					reg.logger().warn('TinyMCE: unsupported Xilinota command: ', cmd);
					return false;
				}

				if (xilinotaCommandToTinyMceCommands[cmd.name] === true) {
					// Already handled in useWindowCommandHandlers.ts
				} else if (xilinotaCommandToTinyMceCommands[cmd.name] === false) {
					// Explicitely not supported
				} else {
					const tinyMceCmd: TinyMceCommand = { ...(xilinotaCommandToTinyMceCommands[cmd.name] as TinyMceCommand) };
	...
	useEffect(() => {
		...
		element.appendChild(document.createTextNode(`
			.xilinota-tinymce .tox-editor-header {
			...
			*/
			...
			.xilinota-tinymce .tox-toolbar__group {
				background-color: ${theme.backgroundColor3};
				padding-top: ${theme.toolbarPadding}px;
				padding-bottom: ${theme.toolbarPadding}px;
			}

			.xilinota-tinymce .tox .tox-edit-area__iframe {
				background-color: ${theme.backgroundColor} !important;
			}

			.xilinota-tinymce .tox .tox-toolbar__primary {
	...
	useEffect(() => {
		...
		const loadEditor = async () => {
			...
			const toolbar = [
				'bold', 'italic', 'xilinotaHighlight', 'xilinotaStrikethrough', 'formattingExtras', '|',
				'link', 'xilinotaInlineCode', 'xilinotaCodeBlock', 'xilinotaAttach', '|',
				'bullist', 'numlist', 'xilinotaChecklist', '|',
				'h1', 'h2', 'h3', 'hr', 'blockquote', 'table', `xilinotaInsertDateTime${toolbarPluginButtons}`,
			];
			...
			const editors = await (window as any).tinymce.init({
				...
				plugins: 'noneditable link xilinotaLists hr searchreplace codesample table',
				noneditable_noneditable_class: 'xilinota-editable', // Can be a regex too
				...
				formats: {
					xilinotaHighlight: { inline: 'mark', remove: 'all' },
					xilinotaStrikethrough: { inline: 's', remove: 'all' },
					xilinotaInsert: { inline: 'ins', remove: 'all' },
					xilinotaSub: { inline: 'sub', remove: 'all' },
					xilinotaSup: { inline: 'sup', remove: 'all' },
				...
				setup: (editor: Editor) => {
					...
					editor.ui.registry.addButton('xilinotaAttach', {
						...
						onAction: async function() {
							editor.execCommand('xilinotaAttach');
						},
					});

					setupToolbarButtons(editor);

					editor.ui.registry.addButton('xilinotaCodeBlock', {
					...
					editor.ui.registry.addToggleButton('xilinotaInlineCode', {
					...
					editor.ui.registry.addButton('xilinotaInsertDateTime', {
					...
					editor.addShortcut('Meta+Shift+9', '', () => editor.execCommand('InsertXilinotaChecklist'));
	...
	const loadDocumentAssets = (editor: any, pluginAssets: any[]) => {
		...
		// See https://github.com/XilinJia/Xilinota/issues/8520
		...
		const loadContent = async () => {
			...
			if (lastOnChangeEventInfo.current.content !== props.content || !resourcesEqual) {
				...
				if (lastOnChangeEventInfo.current.contentKey !== props.contentKey) {
					...
					// https://github.com/XilinJia/Xilinota/issues/3534
	...
	useEffect(() => {
		...
		function onExecCommand(event: any) {
			...
			const changeCommands = ['mceBlockQuote', 'ToggleXilinotaChecklistItem', 'Bold', 'Italic', 'Underline', 'Paragraph'];
		...
		async function onKeyDown(event: any) {
			...
			// https://github.com/XilinJia/Xilinota/issues/4243
		...
		function onPasteAsText() {
			...
			// https://github.com/XilinJia/Xilinota/pull/8351
		...
		editor.on(TinyMceEditorEvents.XilinotaChange, onChangeHandler);
		...
		return () => {
			try {
				...
				editor.off(TinyMceEditorEvents.XilinotaChange, onChangeHandler);
	...
	return (
		<div style={styles.rootStyle} className="xilinota-tinymce">
