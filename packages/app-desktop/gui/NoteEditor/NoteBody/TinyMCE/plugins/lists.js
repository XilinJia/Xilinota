(function (PluginManager, domGlobals, RangeUtils, TreeWalker, VK, DomQuery, Tools, DOMUtils, BookmarkManager) {
    ...
    function isXilinotaChecklistItem(element) {
    ...
    var updateListWithDetails = function (dom, el, detail) {
      ...
      if (detail.listType === 'xilinotaChecklist') {
        ...
        el.classList.remove('xilinota-checklist');
      }
    };
    ...
    var applyList = function (editor, listName, detail) {
      ...
      Tools.each(getSelectedTextBlocks(editor, rng, root), function (block) {
        ...
        if (sibling && isListNode(sibling) && sibling.nodeName === listName && hasCompatibleStyle(dom, sibling, detail)) {
          ...
          if (detail.listType === 'xilinotaChecklist') {
            listBlock.classList.add('xilinota-checklist');
          } else {
            listBlock.classList.remove('xilinota-checklist');
          }
    ...
    var register = function (editor) {
      ...
      addXilinotaChecklistCommands(editor, ToggleList);
    };
    ...
    var setup$2 = function (editor) {
      var editorClickHandler = function (event) {
        if (!isXilinotaChecklistItem(event.target))
          return;
        if (event.offsetX >= 0)
          return;
        editor.execCommand('ToggleXilinotaChecklistItem', false, { element: event.target });
      };
    ...
    function Plugin () {
      PluginManager.add('xilinotaLists', function (editor) {
        setup$1(editor);
        setup$2(editor);
        register$1(editor);
        register(editor);
