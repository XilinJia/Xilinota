...
import SyncTargetRegistry, { SyncTargetInfo } from '@xilinota/lib/SyncTargetRegistry';
import useElementSize from '@xilinota/lib/hooks/useElementSize';
...
export default function(props: Props): React.JSX.Element {
	...
	const onJoplinCloudCreateAccountClick = useCallback(() => {
		void bridge().openExternal('https://xilinotaapp.org/plans/');
	}, []);
	...
	function renderContent() {
		...
		const selfHostingMessage = showJoplinCloudForm ? null : <SelfHostingMessage>Self-hosting? Xilinota also supports various self-hosting options such as Nextcloud, WebDAV, AWS S3 and Xilinota Server. <a href="#" onClick={onSelfHostingClick}>Click here to select one</a>.</SelfHostingMessage>;
