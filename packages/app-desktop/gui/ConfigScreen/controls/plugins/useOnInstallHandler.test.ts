...
import PluginService, { defaultPluginSetting } from '@xilinota/lib/services/plugins/PluginService';
import { ItemEvent } from './PluginBox';

jest.mock('@xilinota/lib/services/plugins/PluginService');
...
describe('useOnInstallHandler', () => {
	...
	beforeAll(() => {
		...
		(defaultPluginSetting as jest.Mock).mockImplementation(
			jest.requireActual('@xilinota/lib/services/plugins/PluginService').defaultPluginSetting,
		);
	});
