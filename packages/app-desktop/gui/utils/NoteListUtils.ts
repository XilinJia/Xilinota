import { utils as pluginUtils, PluginStates } from '@xilinota/lib/services/plugins/reducer';
...
import { getNoteCallbackUrl } from '@xilinota/lib/callbackUrlUtils';
...
export default class NoteListUtils {
	public static makeContextMenu(noteIds: string[], props: ContextMenuProps) {
		...
		if (!hasEncrypted) {
			...
			if (PeerSocket.broadcaster) {
				menu.append(
					new MenuItem(menuUtils.commandToStatefulMenuItem('sendToPeers', noteIds) as any),
				);
			}
