...
import * as ArrayUtils from '@xilinota/lib/ArrayUtils';
...
async function fetchLatestReleases() {
	const response = await shim.fetch('https://objects.xilinotausercontent.com/r/releases');
		...
		if (compareVersions(release.version, packageInfo.version) <= 0) {
			...
			if (shouldSkip) {
				...
				if (buttonIndex === 0) {
					...
					void bridge().openExternal('https://xilinotaapp.org/changelog/');
				}
			}
		}
	} catch (error) {
