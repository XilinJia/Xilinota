...
import { itemIsEncrypted } from '../utils/xilinotaUtils';
...
export default class UserModel extends BaseModel<User> {
	...
	public async checkMaxItemSizeLimit(user: User, buffer: Buffer, item: Item, xilinotaItem: any) {
		...
		const itemTitle = xilinotaItem ? xilinotaItem.title || '' : '';
		const isNote = xilinotaItem && xilinotaItem.type_ === ModelType.Note;
