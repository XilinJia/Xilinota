...
import { resourceBlobPath, serializeXilinotaItem, unserializeXilinotaItem } from '../../utils/xilinotaUtils';
...
describe('shares.folder', () => {
	...
	test('should share a folder with another user', async () => {
		...
		// ----------------------------------------------------------------
		{
			const folder = await unserializeXilinotaItem(itemContent.toString());
			folder.title = 'modified by recipient';
			await updateItem(session2.id, `root:/${folderItem.name}:`, await serializeXilinotaItem(folder));
	...
	test('should see delta changes for linked items', async () => {
		...
		const noteItem = await models().item().loadByJopId(user1.id, '00000000000000000000000000000001');
		const note: NoteEntity = await models().item().loadAsXilinotaItem(noteItem.id);
		await msleep(1);
	...
	// 	await shareWithUserAndAccept(session1.id, session2.id, user2, ShareType.XilinotaRootFolder, folderItem1);
	...
	test('should allow sharing, unsharing and sharing again', async () => {
		...
		// note.
		//
		// Ref: https://discourse.xilinotaapp.org/t/20977
