...
async function fetchUser(path: SubPath, ctx: AppContext): Promise<User> {
	const user = await ctx.xilinota.models.user().load(path.id);
	if (!user) throw new ErrorNotFound(`No user with ID ${path.id}`);
	return user;
}

async function postedUserFromContext(ctx: AppContext): Promise<User> {
	return ctx.xilinota.models.user().fromApiInput(await bodyFields<any>(ctx.req));
}

router.get('api/users/:id', async (path: SubPath, ctx: AppContext) => {
	const user = await fetchUser(path, ctx);
	await ctx.xilinota.models.user().checkIfAllowed(ctx.xilinota.owner, AclAction.Read, user);
	return user;
});
...
router.get('api/users/:id/public_key', async (path: SubPath, ctx: AppContext) => {
	const user = await ctx.xilinota.models.user().loadByEmail(path.id);
	if (!user) return ''; // Don't throw an error to prevent polling the end point

	const ppk = await ctx.xilinota.models.user().publicPrivateKey(user.id);
	if (!ppk) return '';
...
router.post('api/users', async (_path: SubPath, ctx: AppContext) => {
	await ctx.xilinota.models.user().checkIfAllowed(ctx.xilinota.owner, AclAction.Create);
	...
	const output = await ctx.xilinota.models.user().save(user);
	return ctx.xilinota.models.user().toApiOutput(output);
});

router.get('api/users', async (_path: SubPath, ctx: AppContext) => {
	await ctx.xilinota.models.user().checkIfAllowed(ctx.xilinota.owner, AclAction.List);

	return {
		items: await ctx.xilinota.models.user().all(),
		has_more: false,
	};
});
...
router.del('api/users/:id', async (path: SubPath, ctx: AppContext) => {
	const user = await fetchUser(path, ctx);
	await ctx.xilinota.models.user().checkIfAllowed(ctx.xilinota.owner, AclAction.Delete, user);
	await ctx.xilinota.models.user().delete(user.id);
});

router.patch('api/users/:id', async (path: SubPath, ctx: AppContext) => {
	const user = await fetchUser(path, ctx);
	await ctx.xilinota.models.user().checkIfAllowed(ctx.xilinota.owner, AclAction.Update, user);
	const postedUser = await postedUserFromContext(ctx);
	await ctx.xilinota.models.user().save({ id: user.id, ...postedUser });
});
