...
export async function putItemContents(path: SubPath, ctx: AppContext, isBatch: boolean) {
	if (!ctx.xilinota.owner.can_upload) throw new ErrorForbidden('Uploading content is disabled');
	...
	if (isBatch) {
		...
		try {
			...
			if (ctx.query['share_id']) {
				...
				await ctx.xilinota.models.item().checkIfAllowed(ctx.xilinota.owner, AclAction.Create, { jop_share_id: saveOptions.shareId });
			}

			items = [
				{
					name: ctx.xilinota.models.item().pathToName(path.id),
					body: buffer,
				},
			];
		} finally {
	...
	const output = await ctx.xilinota.models.item().saveFromRawContent(ctx.xilinota.owner, items, saveOptions);
	for (const [name] of Object.entries(output)) {
		if (output[name].item) output[name].item = ctx.xilinota.models.item().toApiOutput(output[name].item) as Item;
...
router.get('api/items/:id', async (path: SubPath, ctx: AppContext) => {
	const itemModel = ctx.xilinota.models.item();
	const item = await itemFromPath(ctx.xilinota.owner.id, itemModel, path);
...
router.del('api/items/:id', async (path: SubPath, ctx: AppContext) => {
	try {
		if (path.id === 'root' || path.id === 'root:/:') {
			...
			if (ctx.xilinota.env !== 'dev') throw new ErrorMethodNotAllowed('Deleting the root is not allowed');
			await ctx.xilinota.models.item().deleteAll(ctx.xilinota.owner.id);
		} else {
			const item = await itemFromPath(ctx.xilinota.owner.id, ctx.xilinota.models.item(), path);
			await ctx.xilinota.models.item().checkIfAllowed(ctx.xilinota.owner, AclAction.Delete, item);
			await ctx.xilinota.models.item().deleteForUser(ctx.xilinota.owner.id, item);
		}
	} catch (error) {
...
router.get('api/items/:id/delta', async (_path: SubPath, ctx: AppContext) => {
	const changeModel = ctx.xilinota.models.change();
	return changeModel.delta(ctx.xilinota.owner.id, requestDeltaPagination(ctx.query));
});

router.get('api/items/:id/children', async (path: SubPath, ctx: AppContext) => {
	const itemModel = ctx.xilinota.models.item();
	const parentName = itemModel.pathToName(path.id);
	const result = await itemModel.children(ctx.xilinota.owner.id, parentName, requestPagination(ctx.query));
	return result;
});
