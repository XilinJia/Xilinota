...
import { resourceBlobPath } from '../../utils/xilinotaUtils';
...
describe('api/items', () => {
	...
	test('should create an item', async () => {
		...
		{
			const item: NoteEntity = await models().item().loadAsXilinotaItem(itemId);
	...
	test('should modify an item', async () => {
		...
		const note: NoteEntity = await models().item().loadAsXilinotaItem(item.id);
