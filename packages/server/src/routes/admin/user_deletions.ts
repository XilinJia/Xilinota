...
router.get('admin/user_deletions', async (_path: SubPath, ctx: AppContext) => {
	...
	const page = await ctx.xilinota.models.userDeletion().allPaginated(pagination);
	const users = await ctx.xilinota.models.user().loadByIds(page.items.map(d => d.user_id), { fields: ['id', 'email'] });
