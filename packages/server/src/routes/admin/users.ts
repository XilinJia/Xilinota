...
router.get('admin/users', async (_path: SubPath, ctx: AppContext) => {
	const userModel = ctx.xilinota.models.user();
	await userModel.checkIfAllowed(ctx.xilinota.owner, AclAction.List);
	...
	const page = await ctx.xilinota.models.user().allPaginated(pagination, {
...
router.get('admin/users/:id', async (path: SubPath, ctx: AppContext, user: User = null, error: any = null) => {
	...
	const subscription = !isNew ? await ctx.xilinota.models.subscription().byUserId(userId) : null;
	const isScheduledForDeletion = await ctx.xilinota.models.userDeletion().isScheduledForDeletion(userId);
