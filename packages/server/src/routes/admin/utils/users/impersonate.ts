...
export async function startImpersonating(ctx: AppContext, userId: Uuid) {
	...
	const user = await ctx.xilinota.models.session().sessionUser(adminSessionId);
	...
	const impersonatedSession = await ctx.xilinota.models.session().createUserSession(userId);
