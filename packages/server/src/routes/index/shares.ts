...
import { FileViewerResponse, renderItem as renderXilinotaItem } from '../../utils/xilinotaUtils';
...
async function renderItem(context: AppContext, item: Item, share: Share): Promise<FileViewerResponse> {
	if (item.jop_type === ModelType.Note) {
		return renderXilinotaItem(share.owner_id, item, share, context.query);
	}
...
router.get('shares/:id', async (path: SubPath, ctx: AppContext) => {
	...
	const user = await ctx.xilinota.models.user().load(share.owner_id);
	...
	ctx.xilinota.models.share().checkShareUrl(share, ctx.URL.origin);
