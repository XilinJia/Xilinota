...
function setupMessageHtml() {
	if (config().isJoplinCloud) {
		...
		return `In this screen, select "<strong>Xilinota Server</strong>" as a synchronisation target, then enter the URL <strong>${config().apiBaseUrl}</strong> and your username and password`;
	}
}
...
router.get('home', async (_path: SubPath, ctx: AppContext) => {
	...
	if (ctx.method === 'GET') {
		...
		const subscription = await ctx.xilinota.models.subscription().byUserId(user.id);
		...
		view.content = {
			...
			showBetaMessage: await isBetaUser(ctx.xilinota.models, user.id),
