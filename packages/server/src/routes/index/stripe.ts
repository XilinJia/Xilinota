...
import { findPrice, PricePeriod } from '@xilinota/lib/utils/joplinCloud';
...
async function getSubscriptionInfo(event: Stripe.Event, ctx: AppContext): Promise<SubscriptionInfo> {
	...
	const sub = await ctx.xilinota.models.subscription().byStripeSubscriptionId(stripeSub.id);
...
export const postHandlers: PostHandlers = {
	...
	createCheckoutSession: async (stripe: Stripe, __path: SubPath, ctx: AppContext) => {
		...
		if (fields.email) {
			...
			// period.
			const existingUser = await ctx.xilinota.models.user().loadByEmail(checkoutSession.customer_email);
			if (existingUser && await isBetaUser(ctx.xilinota.models, existingUser.id)) {
		...
		await ctx.xilinota.models.keyValue().setValue(`stripeSessionToPriceId::${session.id}`, priceId);
	...
	webhook: async (stripe: Stripe, _path: SubPath, ctx: AppContext, event: Stripe.Event = null, logErrors = true) => {
		...
		const models = ctx.xilinota.models;
...
const getHandlers: Record<string, StripeRouteHandler> = {
	...
	success: async (stripe: Stripe, _path: SubPath, ctx: AppContext) => {
		try {
			...
			const validationToken = await ctx.xilinota.models.token().generate(user.id);
			...
			return `
				...
				<p><a href="https://xilinotaapp.org">Go back to XilinotaApp.org</a></p>
			`;
		}
	},
	...
	portal: async (stripe: Stripe, _path: SubPath, ctx: AppContext) => {
		if (!ctx.xilinota.owner) throw new ErrorForbidden('Please login to access the subscription portal');

		const sub = await ctx.xilinota.models.subscription().byUserId(ctx.xilinota.owner.id);
