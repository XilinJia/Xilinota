...
router.get('users/:id', async (path: SubPath, ctx: AppContext, formUser: User = null, error: any = null) => {
	const owner = ctx.xilinota.owner;
	...
	await models.user().checkIfAllowed(ctx.xilinota.owner, AclAction.Read, user);
	...
	const subscription = await ctx.xilinota.models.subscription().byUserId(userId);
...
router.post('users/:id/confirm', async (path: SubPath, ctx: AppContext) => {
	...
	try {
		...
		await ctx.xilinota.models.token().checkToken(userId, fields.token);

		const password = checkRepeatPassword(fields, true);

		await ctx.xilinota.models.user().save({ id: userId, password, must_set_password: 0 });
		await ctx.xilinota.models.token().deleteByValue(userId, fields.token);

		const session = await ctx.xilinota.models.session().createUserSession(userId);
		cookieSet(ctx, 'sessionId', session.id);

		await ctx.xilinota.models.notification().add(userId, NotificationKey.PasswordSet);
