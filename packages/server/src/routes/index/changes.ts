...
router.get('changes', async (_path: SubPath, ctx: AppContext) => {
	if (!ctx.xilinota.owner.is_admin) throw new ErrorForbidden();

	const pagination = makeTablePagination(ctx.query, 'updated_time', PaginationOrderDir.DESC);
	const paginatedChanges = await ctx.xilinota.models.change().allByUser(ctx.xilinota.owner.id, pagination);
	const items = await ctx.xilinota.models.item().loadByIds(paginatedChanges.items.map(i => i.item_id), { fields: ['id'] });

	const table: Table = {
		baseUrl: ctx.xilinota.models.change().changeUrl(),
				...
				items: [
					{
						...
						stretch: true,
						url: showItemUrls(config()) ? (items.find(i => i.id === change.item_id) ? ctx.xilinota.models.item().itemContentUrl(change.item_id) : '') : null,
					},
					{
