...
router.get('items', async (_path: SubPath, ctx: AppContext) => {
	...
	const paginatedItems = await ctx.xilinota.models.item().children(ctx.xilinota.owner.id, '', pagination, { fields: ['id', 'name', 'updated_time', 'mime_type', 'content_size'] });

	const table: Table = {
		baseUrl: ctx.xilinota.models.item().itemUrl(),
		requestQuery: ctx.query,
		pageCount: Math.ceil((await ctx.xilinota.models.item().childrenCount(ctx.xilinota.owner.id, '')) / pagination.limit),
		pagination,
		headers: [
			{
				name: 'name',
