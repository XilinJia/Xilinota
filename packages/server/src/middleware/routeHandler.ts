...
export default async function(ctx: AppContext) {
	...
	try {
		const { response: responseObject, path } = await execRequest(ctx.xilinota.routes, ctx);
		...
		if (responseObject instanceof Response) {
			...
			ctx.response.body = await ctx.xilinota.services.mustache.renderView(view, {
				currentPath: path,
				notifications: ctx.xilinota.notifications || [],
				hasNotifications: !!ctx.xilinota.notifications && !!ctx.xilinota.notifications.length,
				owner: ctx.xilinota.owner,
		...
		if (error.httpCode >= 400 && error.httpCode < 500) {
			...
			ctx.xilinota.appLogger().error(line.join(': '));
		} else {
			ctx.xilinota.appLogger().error(userIp(ctx), error);
		}
		...
		if (error.code === 'invalidOrigin') {
			...
			if (ctx.xilinota.env === Env.Dev && error.stack) r.stack = error.stack;
		...
		ctx.xilinota.appLogger().info(`${ctx.request.method} ${ctx.path} (${ctx.response.status}) (${requestDuration}ms)`);
	}
}
