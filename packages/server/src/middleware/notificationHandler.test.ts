...
describe('notificationHandler', () => {
	...
	test('should check admin password', async () => {
...

		{
			...
			expect(ctx.xilinota.notifications.length).toBe(1);
		}

		{
			...
			expect(ctx.xilinota.notifications.length).toBe(0);
		}
	});
	...
	test('should display a banner if the account is disabled', async () => {
		...
		expect(ctx.xilinota.notifications.find(v => v.id === 'accountDisabled')).toBeTruthy();
	});
	...
	test('should display a banner if the email is not confirmed', async () => {
		...
		{
			const ctx = await runNotificationHandler(session.id);
			expect(ctx.xilinota.notifications.find(v => v.id === 'confirmEmail')).toBeTruthy();
		}

		{
			await models().user().save({ id: user.id, email_confirmed: 1 });
			const ctx = await runNotificationHandler(session.id);
			expect(ctx.xilinota.notifications.find(v => v.id === 'confirmEmail')).toBeFalsy();
		}
	});
