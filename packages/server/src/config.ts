import { rtrimSlashes } from '@xilinota/lib/path-utils';
import { Config, DatabaseConfig, DatabaseConfigClient, Env, MailerConfig, RouteType, StripeConfig } from './utils/types';
import * as pathUtils from 'path';
import { loadStripeConfig, StripePublicConfig } from '@xilinota/lib/utils/joplinCloud';
...
export const fullVersionString = (config: Config) => {
	...
	if (config.appVersion !== config.xilinotaServerVersion) output.push(`(Based on Xilinota Server v${config.xilinotaServerVersion})`);
	return output.join(' ');
};
...
export async function initConfig(envType: Env, env: EnvVariables, overrides: any = null) {
	...
	const forkVersion = packageJson.xilinotaServer?.forkVersion;

	config_ = {
		...env,
		appVersion: forkVersion ? forkVersion : packageJson.version,
		xilinotaServerVersion: packageJson.version,
		appName,
		...
		xilinotaAppBaseUrl: env.JOPLINAPP_BASE_URL,
