...
export async function csrfCheck(ctx: AppContext, isPublicRoute: boolean) {
	...
	const userId = ctx.xilinota.owner ? ctx.xilinota.owner.id : '';
	if (!userId) return;
	...
	if (!(await ctx.xilinota.models.token().isValid(userId, fields._csrf))) {
		throw new ErrorForbidden(`Invalid CSRF token: ${fields._csrf}`);
	}

	await ctx.xilinota.models.token().deleteByValue(userId, fields._csrf);
}
...
export async function createCsrfTokenFromContext(ctx: AppContext, throwOnError = true) {
	return createCsrfToken(ctx.xilinota.models, ctx.xilinota.owner, throwOnError);
}
