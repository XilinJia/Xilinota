...
import { initializeXilinotaUtils } from '../xilinotaUtils';
...
export async function beforeAllDb(unitName: string, createDbOptions: CreateDbOptions = null) {
	...
	await initializeXilinotaUtils(config(), models(), mustache);
}
...
export async function credentialFile(filename: string): Promise<string> {
	const filePath = `${require('os').homedir()}/xilinota-credentials/${filename}`;
...
todo_completed: 0
source: xilinotadev-desktop
source_application: ac.mdiq.xilinotadev-desktop
application_data: 
order: 0
