...
import Logger, { LogLevel, TargetType } from '@xilinota/utils/Logger';
...
const reactions: Record<Action, Reaction> = {
	...
	[Action.UpdateNote]: async (context, user) => {
		...
		try {
			...
			const note = await models().item().loadAsXilinotaItem(noteItem.id);
	...
	[Action.UpdateFolder]: async (context, user) => {
		...
		try {
			...
			const folder = await models().item().loadAsXilinotaItem(folderItem.id);
