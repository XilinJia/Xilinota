...
import { StripePublicConfig } from '@xilinota/lib/utils/joplinCloud';
...
export interface AppContext extends Koa.Context {
	xilinota: AppContextXilinota;

	// All the properties under `xilinota` were previously at the root, so to make
...
export interface Config extends EnvVariables {
	appVersion: string;
	xilinotaServerVersion: string; // May be different from appVersion, if this is a fork of JS
	appName: string;
	env: Env;
	port: number;
	rootDir: string;
