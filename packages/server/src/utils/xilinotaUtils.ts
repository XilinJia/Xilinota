...
import { OptionsResourceModel } from '@xilinota/renderer/MarkupToHtml';
import { isValidHeaderIdentifier } from '@xilinota/lib/services/e2ee/EncryptionService';
const { DatabaseDriverNode } = require('@xilinota/lib/database-driver-node.js');
...
const logger = Logger.create('XilinotaUtils');
...
const pluginAssetRootDir_ = require('path').resolve(__dirname, '../..', 'node_modules/@xilinota/renderer/assets');

let db_: XilinotaDatabase = null;
...
export async function initializeXilinotaUtils(config: Config, models: Models, mustache: MustacheService) {
	models_ = models;
	baseUrl_ = config.baseUrl;
	mustache_ = mustache;

	const filePath = `${config.tempDir}/xilinota.sqlite`;
	await fs.remove(filePath);

	db_ = new XilinotaDatabase(new DatabaseDriverNode());
...
export function isXilinotaItemName(name: string): boolean {
	return !!name.match(/^[0-9a-zA-Z]{32}\.md$/);
}

export async function unserializeXilinotaItem(body: string): Promise<any> {
	return BaseItem.unserialize(body);
}

export async function serializeXilinotaItem(item: any): Promise<string> {
...
export function isXilinotaResourceBlobPath(path: string): boolean {
...
async function noteLinkedItemInfos(userId: Uuid, itemModel: ItemModel, noteBody: string): Promise<LinkedItemInfos> {
	...
	for (const jopId of jopIds) {
		...
		output[jopId] = {
			item: itemModel.itemToXilinotaItem(item),
			file: null,
		};
	}
...
async function renderResource(userId: string, resourceId: string, item: Item, content: Buffer): Promise<FileViewerResponse> {
	...
	try {
		...
		jopItem = await models_.item().loadAsXilinotaItem(resourceItem.id);
	} catch (error) {
		logger.error(`Could not load Xilinota item ${resourceId} associated with item: ${item.id}`);
	}
	...
	const renderOptions: any = {
		...
		itemIdToUrl: (itemId: Uuid) => {
			...
			if (item.type_ === ModelType.Note) {
				...
				// disable the link.
				// https://github.com/XilinJia/Xilinota/issues/6531
...
const findParentNote = async (itemTree: TreeItem, resourceId: string) => {
	...
	return models_.item().itemToXilinotaItem(item);
};
...
export async function renderItem(userId: Uuid, item: Item, share: Share, query: RenderItemQuery): Promise<FileViewerResponse> {
	...
	const rootNote: NoteEntity = models_.item().itemToXilinotaItem(item);
	...
	if (query.resource_id) {
		...
		itemToRender = models_.item().itemToXilinotaItem(noteItem);
	} else {
		// ------------------------------------------------------------------------------------------
