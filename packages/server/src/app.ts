...
import Logger, { LogLevel, LoggerWrapper, TargetType } from '@xilinota/utils/Logger';
...
import FsDriverNode from '@xilinota/lib/fs-driver-node';
import { getDeviceTimeDrift } from '@xilinota/lib/ntp';
...
import { initializeXilinotaUtils } from './utils/xilinotaUtils';
...
import { parseEnvFile } from '@xilinota/utils/env';
...
async function main() {
		...
		try {
			...
			if (responseFormat === RouteResponseFormat.Html) {
				...
				try {
					...
					ctx.body = await ctx.xilinota.services.mustache.renderView({
						name: 'error',
						title: 'Error',
						path: 'index/error',
						content: { error },
	...
	// Creates the request-specific "xilinota" context property.
	app.use(async (ctx: AppContext, next: KoaNext) => {
		ctx.xilinota = {
			...ctx.xilinotaBase,
			owner: null,
	...
	if (selectedCommand) {
		...
		await initializeXilinotaUtils(config(), ctx.xilinotaBase.models, ctx.xilinotaBase.services.mustache);

		appLogger().info('Performing main storage check...');
		appLogger().info(await storageConnectionCheck(config().storageDriver, ctx.xilinotaBase.db, ctx.xilinotaBase.models));

		if (config().storageDriverFallback) {
			appLogger().info('Performing fallback storage check...');
			appLogger().info(await storageConnectionCheck(config().storageDriverFallback, ctx.xilinotaBase.db, ctx.xilinotaBase.models));
		}

		appLogger().info('Starting services...');
		await startServices(config(), ctx.xilinotaBase.services);
