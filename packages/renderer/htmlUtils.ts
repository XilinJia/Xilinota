...
class HtmlUtils {
	...
	private isAcceptedUrl(url: string, allowedFilePrefixes: string[]): boolean {
		...
		if (url.startsWith('https://') ||
			...
			url.startsWith('xilinota://') ||
			// We also allow anchors but only with a specific set of a characters.
			// Fixes https://github.com/XilinJia/Xilinota/issues/8286
	...
	public sanitizeHtml(html: string, options: SanitizeHtmlOptions = null) {
		...
		// disabled. More info: https://github.com/XilinJia/Xilinota/issues/3021
		//
		...
		const parser = new htmlparser2.Parser({
			...
			onopentag: (name: string, attrs: Record<string, string>) => {
				...
				// fixes it. https://github.com/XilinJia/Xilinota/issues/5687
