 ...
 (function (global) {
	...
	Style.prototype = {
	  ...
	  setStyle: function (styleName, styleValue) {
		...
		do {
		  ...
		  var style = (length > 0 ? value.substring(index, length) : value.substring(index));
		  if (style.substring(0, style.indexOf(":")).trim() === styleName) {
			value = value.substring(0, index).trim() + (next ? " " + value.substring(next).trim() : "");
			break;
		  }
		  index = next;
		} while (index);
	...
	JSDOMParser.prototype = {
	  ...
	  match: function (str) {
		...
		if (this.html.substring(this.currentChar, strlen).toLowerCase() === str.toLowerCase()) {
	  ...
	  readNode: function () {
		...
		if (!closed) {
		  ...
		  if (!this.match(closingTag)) {
			this.error("expected '" + closingTag + "' and got " + this.html.substring(this.currentChar, closingTag.length));
			return null;
		  }
		}
