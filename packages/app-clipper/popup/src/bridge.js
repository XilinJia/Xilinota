...
class Bridge {
	...
	async init(browser, browserSupportsPromises, store) {
		...
		this.browser_notify = async (command) => {
			...
			if (command.name === 'sendContentToXilinota') {
				const content = convertCommandToContent(command);
				this.dispatch({ type: 'CLIPPED_CONTENT_SET', content: content });
				...
				if (content.parent_id) {
					this.sendContentToXilinota(content);
				}
			}
		...
		this.env_ = backgroundPage ? backgroundPage.xilinotaEnv() : 'prod';
	...
	async checkAuth() {
		...
		//
		// https://github.com/XilinJia/Xilinota/issues/5125#issuecomment-869547421
		...
		return new Promise((resolve, reject) => {
			const checkStatus = () => {
				if (this.clipperServerPortStatus_ === 'not_found') {
					reject(new Error('Could not find clipper service. Please make sure that Xilinota is running and that the clipper server is enabled.'));
					return true;
	...
	async sendContentToXilinota(content) {
		console.info('Popup: Sending to Xilinota...');
		...
		try {
			...
			// results in Xilinota having the same note twice. There's a 2-3 sec delay between
			// each request. The bug only happens the first time the extension popup is open and the
			// Complete button is clicked.
			//
			// It's beyond my understanding how it's happening. I don't know how this sendContentToXilinota function
			// can be called twice. But even if it is, logically, it's impossible that this
			// call below would be done with twice the same nounce. Even if the function sendContentToXilinota
			...
			// attacks), but simply to detect these double-requests and ignore them on Xilinota side.
			//
