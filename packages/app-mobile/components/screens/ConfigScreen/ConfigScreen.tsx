...
class ConfigScreenComponent extends BaseScreenComponent {
	...
	public constructor() {
		...
		this.exportProfileButtonPress_ = async () => {
			...
			const p = this.state.profileExportPath ? this.state.profileExportPath : `${externalDir}/XilinotaProfileExport`;
	...
	public settingToComponent(key: string, value: any) {
		...
		if (md.isEnum) {
			...
			// https://github.com/XilinJia/Xilinota/issues/2733
	...
	public render() {
				...
				<View key="permission_info" style={this.styles().settingContainer}>
					<View key="permission_info_wrapper">
						<Text key="perm1a" style={this.styles().settingText}>
							{_('To work correctly, the app needs the following permissions. Please enable them in your phone settings, in Apps > Xilinota > Permissions')}
						</Text>
		...
		settingComps.push(
			<View key="version_info_app" style={this.styles().settingContainer}>
				<Text style={this.styles().settingText}>{`Xilinota ${VersionInfo.appVersion}`}</Text>
			</View>,
		);
