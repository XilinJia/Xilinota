...
class NoteTagsDialogComponent extends React.Component {
    ...
    constructor(props) {
        ...
        const noteHasTag = (tagId) => {
            for (let i = 0; i < this.state.tagListData.length; i++) {
                if (this.state.tagListData[i].id === tagId)
                    return this.state.tagListData[i].selected;
            }
            return false;
        };
        const newTagTitles = () => {
        ...
        this.renderTag = data => {
            ...
            return (<TouchableOpacity key={tag.id} onPress={() => this.tag_press(tag.id)} style={this.styles().tag}>
    ...
    render() {
        ...
        const dialogContent = (<View style={{ flex: 1 }}>
				<View style={this.styles().tagBox}>
					<Text style={this.styles().newTagBoxLabel}>{_('New tags:')}</Text>
					<TextInput selectionColor={theme.textSelectionColor} keyboardAppearance={theme.keyboardAppearance} value={this.state.newTags} onChangeText={value => {
                this.setState({ newTags: value });
            }} style={this.styles().tagBoxInput} placeholder={_('tag1, tag2, ...')}/>
				</View>
				<View style={this.styles().tagBox}>
					<TextInput selectionColor={theme.textSelectionColor} keyboardAppearance={theme.keyboardAppearance} value={this.state.tagFilter} onChangeText={value => {
                this.setState({ tagFilter: value });
            }} placeholder={_('Filter tags')} style={this.styles().tagBoxInput}/>
				</View>
...
const NoteTagsDialog = connect((state) => {
    return {
        ...
        tags: state.tags,
        noteId: state.selectedNoteIds.length ? state.selectedNoteIds[0] : '',
    };
})(NoteTagsDialogComponent);
export default NoteTagsDialog;
// module.exports = NoteTagsDialog;
//# sourceMappingURL=NoteTagsDialog.js.map