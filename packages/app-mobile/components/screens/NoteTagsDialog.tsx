...
import { TagEntity } from '@xilinota/lib/services/database/types';
...
class NoteTagsDialogComponent extends React.Component<Props, State> {
	styles_: any;
	tag_press: (tagId: any) => void;
	renderTag: (data: any) => React.JSX.Element;
	tagKeyExtractor: (tag: any) => any;
	okButton_press: () => Promise<void>;
	cancelButton_press: () => void;
	filterTags: (allTags: any) => any;
	...
	constructor(props: Props) {
		...
		this.filterTags = (allTags) => {
			return allTags.filter((tag: { title: string; }) => tag.title.toLowerCase().includes(this.state.tagFilter.toLowerCase()), allTags);
		};
	}
	...
	async loadNoteTags(noteId: string) {
		const tags = await Tag.tagsByNoteId(noteId);
		const tagIds = tags.map(t => t.id);

		const tagListData = this.props.tags.map((tag: TagEntity) => {
			return {
				id: tag.id,
				title: tag.title,
		...
		tagListData.sort((a: { selected: boolean; title: string | undefined; }, b: { selected: boolean; title: string | undefined; }) => {
