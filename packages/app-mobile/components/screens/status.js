        ...
        const renderBody = (report) => {
            ...
            for (let i = 0; i < report.length; i++) {
                ...
                for (const n in section.body) {
                    if (!section.body.hasOwnProperty(n))
                        continue;
                    style = { ...baseStyle };
            ...
            return (<FlatList data={lines} renderItem={({ item }) => {
                    ...
                    style.flex = 1;
                    const retryAllButton = item.retryAllHandler ? (<View style={{ flex: 0 }}>
								<Button title={_('Retry All')} onPress={item.retryAllHandler}/>
							</View>) : null;
                    const retryButton = item.retryHandler ? (<View style={{ flex: 0 }}>
