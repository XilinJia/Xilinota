...
interface Props {
	...
	noteSelectionEnabled: boolean|undefined;

}
class NoteItemComponent extends Component<Props> {
	styles_: any;
	...
	render() {
		...
		const checkboxStyle: any = !isTodo ? { display: 'none' } : { color: theme.color };
...
const NoteItem = connect((state: AppState) => {
	return {
