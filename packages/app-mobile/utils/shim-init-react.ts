...
import { ResourceEntity } from '@xilinota/lib/services/database/types';
import FsDriverBase from '@xilinota/lib/fs-driver-base';

const injectedJs: { webviewLib: any; codeMirrorBundle: any; svgEditorBundle: any; } = {
...
export function shimInit() {
	...
	shim.randomBytes = async (count): Promise<number[]> => {
	...
	shim.detectAndSetLocale = (): string => {
		// [
		// 	{
		// 		"countryCode": "US",
		// 		"isRTL": false,
	...
	shim.fetch = async function(url: string, options = null) {
	...
	shim.fetchBlob = async function(url_: string | URL, options) {
		...
		try {
			...
			throw new Error(`fetchBlob: ${method} ${url}: ${(error as any).toString()}`);
		}
	};

	shim.uploadBlob = async function(url: string, options) {
		...
		try {
			...
			throw new Error(`uploadBlob: ${method} ${url}: ${(error as any).toString()}`);
		}
	};
	...
	shim.stringByteLength = function(string): number {
	...
	shim.createResourceFromPath = async function(filePath: string, defaultProps = null): Promise<ResourceEntity> {
	...
	shim.injectedJs = function(name: string) {
	...
	shim.clearTimeout = (id: string | number): void => {
		if (typeof id === 'number') PoorManIntervals.clearTimeout(id);
		else throw ('clearTimeout id not number');
	};

	shim.clearInterval = (id: string | number): void => {
		if (typeof id === 'number') PoorManIntervals.clearInterval(id);
		else throw ('clearInterval id not number');
	};
