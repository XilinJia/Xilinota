...
const { fileExtension, dirname } = require('@xilinota/lib/path-utils');
const wrap_ = require('word-wrap');
const { languageCode } = require('@xilinota/lib/locale');
...
function getHeader() {
	...
	output.push('NAME');
	output.push('');
	output.push(wrap('xilinota - a note taking and to-do app with synchronisation capabilities'), INDENT);
	...
	const description = [];
	description.push('Xilinota is a note taking and to-do application, which can handle a large number of notes organised into notebooks.');
	...
	description.push('\n\n');
	description.push('Notes exported from Evenotes via .enex files can be imported into Xilinota, including the formatted content, resources (images, attachments, etc.) and complete metadata (geolocation, updated time, created time, etc.).');
...
function getFooter() {
	...
	output.push('WEBSITE');
	output.push('');
	output.push(`${INDENT}https://xilinotaapp.org`);
