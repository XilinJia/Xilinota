...
const XilinotaDatabase = require('@xilinota/lib/XilinotaDatabase').default;
...
const xilinotaAppPath = `${__dirname}/main.js`;
...
const db = new XilinotaDatabase(new DatabaseDriverNode());
...
function execCommand(client, command) {
	const exePath = `node ${xilinotaAppPath}`;
...
async function main() {
	...
	for (const n in testUnits) {
		...
		await clearDatabase();
		const testName = n.substring(4).toLowerCase();
