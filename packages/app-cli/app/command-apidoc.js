...
const { toTitleCase } = require('@xilinota/lib/string-utils.js');
...
class Command extends BaseCommand {
	...
	async action(args) {
		...
		lines.push('# Xilinota Data API');
		lines.push('');
		lines.push('This API is available when the clipper server is running. It provides access to the notes, notebooks, tags and other Xilinota object via a REST API. Plugins can also access this API even when the clipper server is not running.');
		lines.push('');
		...
		lines.push('');
		lines.push('To prevent unauthorised applications from accessing the API, the calls must be authentified. To do so, you must provide a token as a query parameter for each API call. You can get this token from the Xilinota desktop application, on the Web Clipper Options screen.');
		lines.push('');
...
async function fetchAllNotes() {
	...
	do {
		...
		lines.push('');
		lines.push('Call **GET /search?query=YOUR_QUERY** to search for notes. This end-point supports the `field` parameter which is recommended to use so that you only get the data that you need. The query syntax is as described in the main documentation: https://xilinotaapp.org/help/#searching');
		lines.push('');
		...
		for (let i = 0; i < models.length; i++) {
			...
			const singular = tableName.substring(0, tableName.length - 1);
			...
			if (model.type === BaseModel.TYPE_RESOURCE) {
				...
				lines.push('');
				lines.push('```javascript');
				lines.push('\tawait xilinota.data.post(');
