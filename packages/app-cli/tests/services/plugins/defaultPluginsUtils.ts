import { installDefaultPlugins, getDefaultPluginsInstallState, setSettingsForDefaultPlugins, checkPreInstalledDefaultPlugins } from '@xilinota/lib/services/plugins/defaultPlugins/defaultPluginsUtils';
import PluginRunner from '../../../app/services/plugins/PluginRunner';
import { pathExists } from 'fs-extra';
import { checkThrow, setupDatabaseAndSynchronizer, supportDir, switchClient } from '@xilinota/lib/testing/test-utils';
import PluginService, { defaultPluginSetting, DefaultPluginsInfo, PluginSettings } from '@xilinota/lib/services/plugins/PluginService';
...
describe('defaultPluginsUtils', () => {
	...
	const pluginsId = ['xilinota.plugin.ambrt.backlinksToNote', 'org.xilinotaapp.plugins.ToggleSidebars'];
	...
	it('should install default plugins with previous default plugins installed', (async () => {
		...
		const testPluginDir = `${supportDir}/pluginRepo/plugins`;
		Setting.setValue('installedDefaultPlugins', ['org.xilinotaapp.plugins.ToggleSidebars']);
	...
	it('should get default plugins install state', (async () => {
		const testCases = [
			{
				...
				'loadingPlugins': [`${testPluginDir}/simple`, `${testPluginDir}/jpl_test/org.xilinotaapp.FirstJplPlugin.jpl`],
			...
			{
				'installedDefaultPlugins': ['org.xilinotaapp.plugins.Simple'],
				'loadingPlugins': [`${testPluginDir}/simple`, `${testPluginDir}/jpl_test/org.xilinotaapp.FirstJplPlugin.jpl`],
		...
		for (const testCase of testCases) {
			...
			const pluginsId = ['org.xilinotaapp.plugins.Simple', 'org.xilinotaapp.FirstJplPlugin'];
