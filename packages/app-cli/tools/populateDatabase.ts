...
// - Update the DB parameters in ~/xilinota-credentials/server.env to use the dev
//   database
...
const processUser = async (userNum: number) => {
	...
	try {
		...
		const profileDir = `${homedir()}/.config/xilinotadev-populate/xilinotadev-testing-${userNum}`;
