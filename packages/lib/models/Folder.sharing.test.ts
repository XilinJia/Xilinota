...
describe('models/Folder.sharing', () => {
	...
	it('should apply the share ID to all children', (async () => {
		...
		if (folder) await Folder.save({ id: folder.id, share_id: 'abcd1234' });
	...
	it('should apply the share ID to all sub-folders', (async () => {
		...
		if (folder1) await Folder.save({ id: folder1.id, share_id: 'abcd1234' });
		...
		if (folder1) expect(folder1.share_id).toBe('abcd1234');
	...
	it('should automatically duplicate resources when they are shared', async () => {
		...
		const userUpdatedTimes: Record<string, number> = {
			[note1.id!]: note1.user_updated_time??0,
			[note2.id!]: note2.user_updated_time??0,
			[note3.id!]: note3.user_updated_time??0,
			[note4.id!]: note4.user_updated_time??0,
		};
