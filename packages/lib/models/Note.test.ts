...
describe('models/Note', () => {
	...
	it('should delete only the selected notes', (async () => {
		...
		await createNTestNotes(noOfNotes, f2, [], 'note1');
	...
	it('should convert resource paths from internal to external paths', (async () => {
		...
		{
			const result = await Note.replaceResourceExternalToInternalLinks(`[](xilinota://${note1.id})`);
...

		{
			...
			//
			// https://github.com/XilinJia/Xilinota/issues/5034
