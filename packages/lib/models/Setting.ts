...
class Setting extends BaseModel {
	...
	public static metadata(): SettingItems {
		...
		const emptyDirWarning = _('Attention: If you change this location, make sure you copy all your content to it before syncing, otherwise all files will be removed! See the FAQ for more details: %s', 'https://xilinotaapp.org/faq/');
		...
		this.buildInMetadata_ = {
			...
			'sync.9.path': {
				...
				section: 'sync',
				show: (settings: any) => {
					return settings['sync.target'] === SyncTargetRegistry.nameToId('xilinotaServer');
				},
				public: true,
				label: () => _('Xilinota Server URL'),
				description: () => emptyDirWarning,
				storage: SettingStorage.File,
			},
			...
			'sync.9.password': {
				...
				public: true,
				label: () => _('Xilinota Server password'),
				secure: true,
			},

			// Although sync.10.path is essentially a constant, we still define
			// it here so that both Xilinota Server and Joplin Cloud can be
			...
			shutdownTime: { value: 0, type: SettingItemType.Int, public: false },
			...
			// See https://github.com/XilinJia/Xilinota/pull/6823
			...
			showTrayIcon: {
				...
				description: () => {
					return platform === 'linux' ? _('Note: Does not work in all desktop environments.') : _('This will allow Xilinota to run in the background. It is recommended to enable this setting so that your notes are constantly being synchronised, thus reducing the number of conflicts.');
				},
				storage: SettingStorage.File,
				isGlobal: true,
			},
			...
			privateCode: {
				...
				isGlobal: true,
				public: true,
				label: () => _('Private passcode'),
				description: () => _('This is used for instant data sharing across your devices in the same network. Enter the same code on all of your devices.'),
				storage: SettingStorage.File,
			},
			...
			'style.customCss.xilinotaApp': {
				value: null,
				onClick: () => {
					shim.openOrCreateFile(
						this.customCssFilePath(Setting.customCssFilenames.JOPLIN_APP),
						`/* For styling the entire Xilinota app (except the rendered Markdown, which is defined in \`${Setting.customCssFilenames.RENDERED_MARKDOWN}\`) */`,
					);
				},
				type: SettingItemType.Button,
				public: true,
				appTypes: [AppType.Desktop],
				label: () => _('Custom stylesheet for Xilinota-wide app styles'),
				section: 'appearance',
				advanced: true,
			...
			'net.customCertificates': {
				...
				show: (settings: any) => {
					return [
						...
						SyncTargetRegistry.nameToId('xilinotaServer'),
			...
			'revisionService.enabled': { section: 'revisionService', storage: SettingStorage.File, value: false, type: SettingItemType.Bool, public: true, label: () => _('Enable note history') },
			...
			//
			// - https://discourse.xilinotaapp.org/t/on-android-when-xilinota-gets-started-offline/29951/1
			// - https://github.com/XilinJia/Xilinota/issues/7956
	...
	public static settingMetadata(key: string): SettingItem {
		...
		if (!(key in metadata)) throw new XilinotaError(`Unknown key: ${key}`, 'unknown_key');
	...
	public static value(key: string) {
		...
		for (let i = 0; i < this.cache_.length; i++) {
			if (this.cache_[i].key === key) {
				// logger.debug('Setting value', key, this.cache_[i].value);
	...
	public static async saveAll() {
		...
		for (let i = 0; i < this.cache_.length; i++) {
			// if (this.cache_[i].key === 'lastTimeAlive') {
			// 	this.cache_[i].value = this.formatValue('lastTimeAlive', new Date().getTime());
			// }
