...
export const checkIfItemCanBeChanged = (itemType: ModelType, changeSource: number, item: ItemSlice, shareState: ShareState): void => {
	...
	if (itemIsReadOnlySync(itemType, changeSource, item, Setting.value('sync.userId'), shareState)) {
		throw new XilinotaError(`Cannot change or delete a read-only item: ${item.id}`, ErrorCode.IsReadOnly);
	}
};
...
export const checkIfItemCanBeAddedToFolder = async (itemType: ModelType, Folder: any, changeSource: number, shareState: ShareState, parentId: string): Promise<void> => {
	if (needsReadOnlyChecks(itemType, changeSource, shareState) && parentId) {
		...
		if (itemIsReadOnlySync(itemType, changeSource, parentFolder, Setting.value('sync.userId'), shareState)) {
			throw new XilinotaError('Cannot add an item as a child of a read-only item', ErrorCode.IsReadOnly);
		}
	}
};
...
export const itemIsReadOnly = async (BaseItem: any, itemType: ModelType, changeSource: number, itemId: string, userId: string, shareState: ShareState): Promise<boolean> => {
	...
	if (!item) throw new XilinotaError(`No such item: ${itemType}: ${itemId}`, ErrorCode.NotFound);
