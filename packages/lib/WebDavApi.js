...
const XilinotaError = require('./XilinotaError').default;
...
class WebDavApi {
	...
	authToken() {
		...
		try {
			// Note: Non-ASCII passwords will throw an error about Latin1 characters - https://github.com/XilinJia/Xilinota/issues/246
		...
		const nameProcessor = name => {
			if (name.indexOf('xmlns') !== 0) {
				...
				if (p.length === 2) {
					...
					// https://github.com/XilinJia/Xilinota/issues/2002
					name = `d:${name}`;
				}
			}
	...
	resourcePropByName(resource, outputType, propName) {
		...
		if (outputType === 'string') {
			if (!output) throw new XilinotaError(`String property not found: ${propName}: ${JSON.stringify(resource)}`, 'stringNotFound');
	...
	handleNginxHack_(jsonResponse, newErrorHandler) {
		...
		// https://github.com/XilinJia/Xilinota/issues/624
		// https://github.com/XilinJia/Xilinota/issues/808
		// Not tested but someone confirmed it worked - https://github.com/XilinJia/Xilinota/issues/808#issuecomment-443552858
		...
		// https://github.com/XilinJia/Xilinota/issues/1137
		//
		// <?xml version='1.0' encoding='UTF-8'?>
		// <ns0:multistatus xmlns:ns0="DAV:">
		// 	<ns0:response>
		// 		<ns0:href>/seafdav/xilinota/</ns0:href>
		// 		<ns0:propstat>
		// 			<ns0:prop>
		// 				<ns0:getlastmodified/>
		// 			</ns0:prop>
	...
	async exec(method, path = '', body = null, headers = null, options = null) {
		...
		// React-native has caching enabled by at least on Android (see https://github.com/XilinJia/Xilinota/issues/4706 and the related PR).
		...
		if (['GET', 'HEAD'].indexOf(method) < 0) headers['If-None-Match'] = `XilinotaIgnore-${Math.floor(Math.random() * 100000)}`;
		if (!headers['User-Agent']) headers['User-Agent'] = 'Xilinota/1.0';
