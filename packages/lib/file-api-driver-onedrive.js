	...
	async delta_BROKEN(path, options = null) {
		...
		try {
			...
			if (error.code === 'resyncRequired') {
				...
				// Code: resyncRequired
				// Request: GET https://graph.microsoft.com/v1.0/drive/root:/Apps/XilinotaDev:/delta?select=...
		...
		// The delta API might return things that happens in subdirectories and outside of the xilinota directory.
		// We don't want to deal with these since all the files we're interested in are at the root of the xilinota directory
		// (The .resource dir is special since it's managed directly by the clients and resources never change - only the
		// associated .md file at the root is synced). So in the loop below we check that the parent is indeed the xilinota
