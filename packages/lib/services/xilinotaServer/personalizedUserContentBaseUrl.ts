...
export default function(userId: string, baseUrl: string, userContentBaseUrl: string) {
	...
	// if (userContentBaseUrl === 'http://joplincloud.local:22300') return 'http://abcd1234.xilinotausercontent.local:22300';
	...
	if (userContentBaseUrl && baseUrl !== userContentBaseUrl) {
		...
		return `${url.protocol}//${userId.substring(0, 10).toLowerCase()}.${url.host}`;
	} else {
		return baseUrl;
	}
}
