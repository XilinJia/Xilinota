	...
	for (const searchType of [SearchEngine.SEARCH_TYPE_FTS, SearchEngine.SEARCH_TYPE_NONLATIN_SCRIPT]) {
		...
		describe(`search type ${searchType}`, () => {
			...
			it('should support filtering by sourceurl', (async () => {
				const n0 = await Note.save({ title: 'n0', source_url: 'https://discourse.xilinotaapp.org' });
				...
				const n3 = await Note.save({ title: 'n3', source_url: 'https://xilinotaapp.org' });

				await engine.syncTables();

				let rows = await engine.search('sourceurl:https://xilinotaapp.org', { searchType });
				expect(rows.length).toBe(1);
				expect(ids(rows)).toContain(n3.id);
				...
				rows = await engine.search('sourceurl:*xilinotaapp.org', { searchType });
				expect(rows.length).toBe(2);
				expect(ids(rows)).toContain(n0.id);
				expect(ids(rows)).toContain(n3.id);
