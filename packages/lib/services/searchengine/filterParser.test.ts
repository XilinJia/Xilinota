...
describe('filterParser should be correct filter for keyword', () => {
	...
	it('single word text', () => {
		const searchString = 'xilinota';
		expect(filterParser(searchString)).toContainEqual(makeTerm('text', '"xilinota"', false));
	});

	it('multi word text', () => {
		const searchString = 'scott xilinota';
	...
	it('negated word text', () => {
		const searchString = 'scott -xilinota';
		expect(filterParser(searchString)).toContainEqual(makeTerm('text', '"scott"', false));
		expect(filterParser(searchString)).toContainEqual(makeTerm('text', '"xilinota"', true));
	});

	it('phrase text search', () => {
		const searchString = '"scott xilinota"';
		expect(filterParser(searchString)).toContainEqual(makeTerm('text', '"scott xilinota"', false, true));
	});
