...
export default class ResourceEditWatcher {
		...
		if (!this.watcher_) {
			...
			this.watcher_.on('all', (event: any, path: string) => {
				...
				if (event === 'unlink') {
					...
					// See: https://github.com/XilinJia/Xilinota/issues/710#issuecomment-420997167
			...
			// https://github.com/XilinJia/Xilinota/issues/3407
			//
	...
	public async stopWatching(resourceId: string) {
		...
		try {
			...
			this.logger().warn(`ResourceEditWatcher: There was an error unwatching resource ${resourceId}. Xilinota will ignore the file regardless.`, error);
		}
