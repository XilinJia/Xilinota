...
const createShallowObjectEqualSelector = createSelectorCreator(
	defaultMemoize,
	(prev: any, next: any) => {
		if (!prev || !next || Object.keys(prev).length !== Object.keys(next).length) return false;
		for (const n in prev) {
...
const selectObjectByCommands = createCachedSelector(
	...
	keySelector: (_state: any, commandNames: string[]) => {
		// console.log('selectObjectByCommands', commandNames);
		// commandNames = Object.keys(_state);
		return commandNames ? commandNames.join('_') : '';
	},
...
export default class MenuUtils {
	...
	public commandToMenuItem(commandName: string, onClick: Function): MenuItem {
		...
		const item: MenuItem = {
			id: command ? command.declaration.name : '',
			label: this.service.label(commandName),
			click: () => onClick(command ? command.declaration.name : ''),
			enabled: true,
		};

		if (command && command.declaration.role) item.role = command.declaration.role;
	...
	public commandsToMenuItemProps(commandNames: string[], whenClauseContext: any): MenuItemProps {
		...
		for (const commandName of commandNames) {
			...
			if (!newProps || propsHaveChanged(this.menuItemPropsCache_[commandName], newProps)) {
		...
		// TODO: very strange!!! when import re-reselect, this is not valid
		return selectObjectByCommands({ array: output }, commandNames);
		// return selectObjectByCommands({ array: output });
	}
