...
export interface AcquireLockOptions {
	...
	//
	// https://discourse.xilinotaapp.org/t/error-upgrading-to-2-3-3/19549/4?u=laurent
	...
	private async acquireSyncLock(clientType: LockClientType, clientId: string): Promise<Lock> {
		...
		try {
			...
			while (true) {
				...
				if (exclusiveLock) {
					throw new XilinotaError(`Cannot acquire sync lock because the following client has an exclusive lock on the sync target: ${this.lockToClientString(exclusiveLock)}`, 'hasExclusiveLock');
				}
			...
			while (true) {
				...
				if (activeSyncLock) {
					if (options.clearExistingSyncLocksFromTheSameClient && activeSyncLock.clientId === clientId && activeSyncLock.clientType === clientType) {
						...
						throw new XilinotaError(`Cannot acquire exclusive lock because the following clients have a sync lock on the target: ${this.lockToClientString(activeSyncLock)}`, 'hasSyncLock');
					}
				}
				...
				if (activeExclusiveLock) {
					if (activeExclusiveLock.clientId === clientId) {
						...
						if (await waitForTimeout()) continue;
						throw new XilinotaError(`Cannot acquire exclusive lock because the following client has an exclusive lock on the sync target: ${this.lockToClientString(activeExclusiveLock)}`, 'hasExclusiveLock');
					}
				} else {
	...
	public startAutoLockRefresh(lock: Lock, errorHandler: Function): string {
		...
		this.refreshTimers_[handle].id = shim.setInterval(async () => {
			...
			if (!hasActiveLock(locks, await this.currentDate(), this.lockTtl, lock.type, lock.clientType, lock.clientId)) {
				...
				error = new XilinotaError('Lock has expired', 'lockExpired');
			} else {
				try {
