...
export function replaceRegexDiacritics(regexString: string): string {
...
export function escapeFilename(s: string, maxLength: number = 32): string {
...
export function wrap(text: any, indent: string | string[], width: number) {
...
export function commandArgumentsToString(args: string | string[]): string {
	const output = [];
...
export function splitCommandBatch(commandBatch: string | string[]): string[] {
	const commandLines = [];
	const eol = '\n';
...
export function padLeft(str: string, length: number, padString: any): string {
	if (!str) return '';
...
export function surroundKeywords(keywords: any[], text: string, prefix: string, suffix: string, options: any = null): string {
	...
	// 	.map((k: { type: string; valueRegex: any; value: any; }) => {
	// 		if (k.type === 'regex') {
...
export function substrWithEllipsis(s: string, start: any, length: number): string {
	if (s.length <= length) return s;
	return `${s.substring(start, length - 3)}...`;
}

export function nextWhitespaceIndex(s: string, begin: number): number {
	// returns index of the next whitespace character
	const i = s.slice(begin).search(/\s/);
	return i < 0 ? s.length : begin + i;
}

export function camelCaseToDash(s: string | string[]): string {
	const output = [];
...
export function formatCssSize(v: string | string[]): string {
	if (typeof v === 'string') {
...
export function scriptType(s: string): "zh" | "ja" | "ko" | "th" | "en" {
