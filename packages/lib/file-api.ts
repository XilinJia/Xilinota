...
export interface RemoteItem {
	...
	// example for the locking mechanim or any file that's not an actual Xilinota
	// item.
	updated_time?: number;

	// This is the time that corresponds to the actual Xilinota item updated_time
	...
	// exact Xilinota item updated_time value.
...
class FileApi {
	...
	// probably only be supported by Xilinota Server.
	...
	//
	// The only reliable one at the moment is Xilinota Server since it reads the
	...
	public async stat(path_: string, isFullPhath = false) {
		// logger.debug(`stat ${this.fullPath(path_)}`);
		const fullPath = isFullPhath ? path_ : this.fullPath(path_);
		const output = await tryAndRepeat(() => this.driver_.stat(fullPath), this.requestRepeatCount());
	...
	public async put(path_: string, content: any, options: any = null) {
		...
		if (options && options.source === 'file') {
			if (!(await this.fsDriver().exists(options.path))) throw new XilinotaError(`File not found: ${options.path}`, 'fileNotFound');
		}
	...
	public delete(path_: string, isFullPhath = false) {
		const fullPath = isFullPhath ? path_ : this.fullPath(path_);
		// logger.debug(`delete ${fullPath}`);
		return tryAndRepeat(() => this.driver_.delete(fullPath), this.requestRepeatCount());
	}
...
async function basicDelta(path_: string, getDirStatFn: Function, options: any) {
	...
	if (!newContext.deletedItemsProcessed) {
		...
		if (options.wipeOutFailSafe && percentDeleted >= 0.90) throw new XilinotaError(sprintf('Fail-safe: Sync was interrupted because %d%% of the data (%d items) is about to be deleted. To override this behaviour disable the fail-safe in the sync settings.', Math.round(percentDeleted * 100), deletedItems.length), 'failSafe');
