...
export default class Synchronizer {
	...
	protected async apiCall(fnName: string, ...args: any[]) {
		if (this.syncTargetIsLocked_) throw new XilinotaError('Sync target is locked - aborting API call', 'lockError');
			...
			if (syncSteps.indexOf('update_remote') >= 0) {
				...
				while (true) {
					...
					for (let i = 0; i < locals.length; i++) {
						...
						if (donePaths.indexOf(path) >= 0) throw new XilinotaError(sprintf('Processing a path that has already been done: %s. sync_time was not updated? Remote item has an updated_time in the future?', path), 'processingPathTwice');
