...
export const checkProviderIsSupported = (path: string): void => {
	...
	for (const p of unsupportedProviders) {
		if (pathContainsUnsupportedProvider(path, unsupportedProviders)) {
			throw new Error(_('The WebDAV implementation of %s is incompatible with Xilinota, and as such is no longer supported. Please use a different sync method.', p));
		}
	}
};
