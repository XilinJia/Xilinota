...
describe('checkProviderIsSupported', () => {
	describe('when no unsupported provider is already configured', () => {
		...
		it('should throw an error with the name of the provider when an unsupported provider path is provided', () => {
			expect(() => checkProviderIsSupported('https://pcloud.com')).toThrowError('The WebDAV implementation of pcloud is incompatible with Xilinota, and as such is no longer supported. Please use a different sync method.');

			expect(() => checkProviderIsSupported('https://api.pcloud.com')).toThrowError('The WebDAV implementation of pcloud is incompatible with Xilinota, and as such is no longer supported. Please use a different sync method.');

			// expect(() => checkProviderIsSupported('https://api-pcloud-test.com')).toThrowError('The WebDAV implementation of pcloud is incompatible with Xilinota, and as such is no longer supported. Please use a different sync method.');
		});
		// expect(() => checkProviderIsSupported('?param=pcloud')).toThrowError('The WebDAV implementation of pcloud is incompatible with Xilinota, and as such is no longer supported. Please use a different sync method.');
	});
