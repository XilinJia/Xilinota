...
class FileApiDriverWebDav {
	...
	statFromResource_(resource, path) {
		...
		// Disabled for now to try to fix this: https://github.com/XilinJia/Xilinota/issues/624
		//
		// const httpStatusLine = this.api().stringFromJson(resource, ['d:propstat',0,'d:status', 0]);
		// if ( typeof httpStatusLine === 'string' && httpStatusLine.indexOf('404') >= 0 ) throw  new XilinotaError(resource, 404);
	...
	hrefToRelativePath_(href, baseUrl, relativeBaseUrl) {
		...
		if (href.indexOf(baseUrl) === 0) {
			output = href.substring(baseUrl.length);
		} else if (href.indexOf(relativeBaseUrl) === 0) {
			output = href.substring(relativeBaseUrl.length);
		} else {
	...
	statsFromResources_(resources) {
		...
		for (let i = 0; i < resources.length; i++) {
			...
			// const path = rtrimSlashes(ltrimSlashes(href.substring(relativeBaseUrl.length)));
	...
	async list(path) {
		...
		const stats = this.statsFromResources_(resources).map((stat) => {
			if (path && stat.path.indexOf(`${path}/`) === 0) {
				const s = stat.path.substring(path.length + 1);
				if (s.split('/').length === 1) {
					return {
						...stat,
						path: stat.path.substring(path.length + 1),
					};
				}
			}
			return stat;
	...
	async get(path, options) {
		...
		try {
			...
			if (response === 'The specified file doesn\'t exist.') throw new XilinotaError(response, 404);
			return response;
		} catch (error) {
