...
import Logger, { TargetType, LoggerWrapper, LogLevel } from '@xilinota/utils/Logger';
...
const databases_: XilinotaDatabase[] = [];
...
function setSyncTargetName(name: string) {
	...
	isNetworkSyncTarget_ = ['nextcloud', 'dropbox', 'onedrive', 'amazon_s3', 'xilinotaServer', 'joplinCloud'].includes(syncTargetName_);
	synchronizers_ = [];
	return previousName;
}
...
// setSyncTargetName('xilinotaServer');
...
async function setupDatabase(id: number = 0, options: any = null) {
	...
	databases_[id] = new XilinotaDatabase(new DatabaseDriverNode());
...
async function initFileApi() {
	...
	if (syncTargetId_ === SyncTargetRegistry.nameToId('filesystem')) {
		...
		// Then select "XilinotaTest" and click "Generated access token"
		...
		// https://login.live.com/oauth20_authorize.srf?client_id=f1e68e1e-a729-4514-b041-4fdd5c7ac03a&scope=files.readwrite,offline_access&response_type=token&redirect_uri=https://xilinotaapp.org
		//
		// Business OneDrive Account:
		// https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=f1e68e1e-a729-4514-b041-4fdd5c7ac03a&scope=files.readwrite offline_access&response_type=token&redirect_uri=https://xilinotaapp.org
		//
	...
	} else if (syncTargetId_ === SyncTargetRegistry.nameToId('xilinotaServer') || syncTargetId_ === SyncTargetRegistry.nameToId('joplinCloud')) {
		mustRunInBand();

		const xilinotaServerAuth = JSON.parse(await readCredentialFile('xilinota-server-test-units-2.json'));

		// const xilinotaServerAuth = {
