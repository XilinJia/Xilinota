...
export default class XilinotaServerApi {
	...
	public static connectionErrorMessage(error: any) {
		...
		return _('Could not connect to Xilinota Server. Please check the Synchronisation options in the config screen. Full error was:\n\n%s', msg);
	}
	...
	private async exec_(method: string, path = '', query: Record<string, any> = {}, body: any = null, headers: any = null, options: ExecOptions = {}) {
		...
		try {
			...
			const newError = (message: string, code = 0) => {
				...
				return new XilinotaError(message, code, `${method} ${path}: ${message} (${code}): ${shortResponseText()}`);
			};
