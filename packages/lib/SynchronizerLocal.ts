...
export default class SynchronizerLocal extends Synchronizer {
	...
	public async start(options: any = null) {
		...
		const do_DB_FS_diff = async () => {
			...
			const fileSet: Set<string> = new Set(fileList.filter((it: string) => it[0] !== '.' && it[0] !== '_' && LocalFile.isMDFile(it)));
			...
			const do_commons = async () => {
				...
				for (const p of commonLP) {
					...
					// time different
					// Welcome!/5. Xilinota Privacy Policy.md remote: {path: 'Welcome!/5. Xilinota Privacy Policy.md', updated_time: 1697392394529, isDir: false} local: 1697389941913
				...
				for (const p of diffRL) {
					...
					if (LocalFile.isMDFile(p)) {
						...
						if (content) {
							...
							const { frontMatter } = BaseItem.parseFrontMatter(content);
		...
		const do_indexing = async () => {
			...
			//
			// https://github.com/XilinJia/Xilinota/issues/932#issuecomment-933736405
			try {
		...
		const do_delete = async () => {
			...
			for (let i = 0; i < deletedItems.length; i++) {
				...
				if (!isResource) {
					...
					path = await LocalFile.sysPathFromRoot(itemR);
				} else {
			...
			if (syncSteps.indexOf('update_remote') >= 0) {
				...
				while (true) {
					...
					for (let i = 0; i < locals.length; i++) {
						...
						const pathFromRoot = await LocalFile.sysPathFromRoot(local, createDir);
						if (donePaths.indexOf(pathFromRoot) >= 0) throw new XilinotaError(sprintf('Processing a path that has already been done: %s. sync_time was not updated? Remote item has an updated_time in the future?', local.title), 'processingPathTwice');
					...
					for (let i = 0; i < locals.length; i++) {
						...
						if (local.type_ === BaseModel.TYPE_FOLDER) continue;
						const ItemClass: typeof BaseItem = BaseItem.itemClass(local);
						const path = LocalFile.fileNameFS(local);
						...
						const pathFromRoot = await LocalFile.sysPathFromRoot(local, createDir);
						if (donePaths.indexOf(pathFromRoot) >= 0) throw new XilinotaError(sprintf('Processing a path that has already been done: %s. sync_time was not updated? Remote item has an updated_time in the future?', pathFromRoot), 'processingPathTwice');
						const remote: RemoteItem = result.neverSyncedItemIds.includes(local.id) ? null : await this.apiCall('stat', pathFromRoot);
						let action = null;
						...
						if (action === 'createRemote' || action === 'updateRemote') {
							let canSync = true;
							try {
								if (this.testingHooks_.indexOf('notesRejectedByTarget') >= 0 && local.type_ === BaseModel.TYPE_NOTE) throw new XilinotaError('Testing rejectedByTarget', 'rejectedByTarget');
			...
			if (syncSteps.indexOf('delta') >= 0) {
				...
				while (true) {
					...
					for (const remote of remotes) {
						...
						if (local) {
							if (local.type_ === BaseModel.TYPE_NOTE) {
								romotePathFromRoot = await LocalFile.sysPathFromRoot(local, createDir);
							} else {
								continue;
							}
						}
					...
					for (let i = 0; i < remotes.length; i++) {
						...
						if (!LocalFile.isMDFile(remote.path)) continue; // The delta API might return things like the .sync, .resource or the root folder
						...
						if (local) {
							if (local.type_ === BaseModel.TYPE_NOTE) {
								path = await LocalFile.sysPathFromRoot(local);
							}
						}
