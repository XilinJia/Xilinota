	...
	return [
		{
			...
			text: `
				...
				*/
				...
				ul.xilinota-checklist li::before {
					content:"\\f14a";
				...
				.xilinota-checklist li:not(.checked)::before {
					content:"\\f0c8";
				}`,
		},
	];
		...
		for (let i = 0; i < tokens.length; i++) {
			...
			if (currentListItem && currentListItem.markup === '-' && !processedFirstInline && token.type === 'inline') {
				...
				if (renderingType === 1) {
					...
					itemClass += ' md-checkbox xilinota-checkbox';
					...
					if (listClass.indexOf('xilinota-') < 0) currentList.attrSet('class', (`${listClass} xilinota-checklist`).trim());
